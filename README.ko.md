...
`@ilokesto/fetcher`는 그 특정 `ky` 에러에 한해서 leading slash를 제거한 뒤 다시 시도합니다. 그래서 OpenAPI 스타일 route literal을 `prefixUrl`과 함께 그대로 쓸 수 있습니다.
...
패키지는 `@ilokesto/fetcher/openapi`에서 다음 주요 helper type을 export합니다.
